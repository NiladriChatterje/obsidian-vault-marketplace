...
export async function inspectVaultZip(file: File): Promise<VaultZipCheck> {
  ...
  try {
    ...
    return { ok: false, noteCount: 0, attachmentCount: 0, reason: 'That file is not a valid zip archive.' };
  }

  const paths = Object.keys(entries);
  if (!paths.length) return { ok: false, noteCount: 0, attachmentCount: 0, reason: 'That zip is empty.' };
  ...
  if (noteCount > 0) return { ok: true, noteCount, attachmentCount };
  ...
  return { ok: false, noteCount: 0, attachmentCount, reason };
}
